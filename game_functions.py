...
def update_screen(ai_settings, screen, stats, sb, ship, aliens, bullets,
    ...
    # Отрисовка информации о количестве очков.
    sb.show_score()
    
    # Отрисовка кнопки Play, если игра не начата.
...
def update_bullets(ai_settings, screen, stats, sb, ship, aliens, bullets):
    """Обновление позиций пуль и удаление пуль вышедших за предел экрана."""
    bullets.update()
...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship,
        aliens, bullets):
    """Проверка столкновения пули и пришельца."""
    # Удаление пули и пришельца если они сталкиваются.
    ...
    if len(aliens) == 0:
        # Начинаем новый уровень, если весь флот уничтожен.
        bullets.empty()
...
def change_fleet_direction(ai_settings, aliens):
    """Снижаем весь флот и меняем направление его движения."""
    for alien in aliens.sprites():
...
def update_aliens(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    после чего обновляем позиции всех пришельцев во флоте.
    """
    check_fleet_edges(ai_settings, aliens)
    aliens.update()
    
    # Проверка столкновений корабля и пришельцев.
...
def create_alien(ai_settings, screen, aliens, alien_number, row_number):
    """Создаем нового пришельца и размещаем его в ряду."""
